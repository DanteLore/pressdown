...
def convert_content(c):
    ...
    c = re.sub(r'<li class="blocks-gallery-item">.*?<img.*?src="([^\"]*)".*?/>.*?</li>', '<img src="\g<1>" class="gallery"/>', c, flags=re.MULTILINE)
    c = re.sub(r'<em>(.*?)</em>', '*\g<1>*', c, flags=re.MULTILINE)
    c = re.sub(r'<del>(.*?)</del>', '~~\g<1>~~', c, flags=re.MULTILINE)
    c = re.sub(r'<a.*href="([^\"]*)".*?>(.*?)</a>', '[\g<2>](\g<1>)', c, flags=re.MULTILINE)
    c = re.sub(r'<code.*?>([\S\s]*?)</code>', '```\n\g<1>\n```', c, flags=re.MULTILINE)
    c = re.sub(r'<br.*?/>', '\n', c, flags=re.MULTILINE)
    c = re.sub(r'\n\n\n', r'\n', c, flags=re.MULTILINE)
    return c
